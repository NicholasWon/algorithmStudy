...
let n = Int(readLine()!)! // 26

func getPCycle(_ i: Int, count c: Int) -> Int {
    let input = (i/10 + i%10)%10 + i%10*10 // (2 + 6)%10 -> 8, 26%10*10 -> 60 => 68
    let count = c+1 // 카운트 1 증가
    return (input == n) ? count : getPCycle(input, count: count) // input 값이 26과 같을 때까지 재귀연산
}

print(getPCycle(n, count: 0))
